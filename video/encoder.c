		      ...
		      u32 width, u32 height, const struct ave_enc_params *params)
{
	...
	/* Optional properties below tolerate a rejection (-EIO response) but
	 * treat a timeout as fatal: no response means the T2 stopped
	 * talking, every later command would burn its full timeout, and
	 * each stranded Q1 buffer leaks a ring slot. */
	...
	if (status) {
		pr_err("[setup] MVHEVCVideoLayerIDs FAILED (%d)\n", status);
		goto fail_queues;
	}
...
fail_queues:
	...
	/* Queues are gone — leave the session in IDLE so a stray teardown
	 * call is a no-op. The caller frees the session. */
	session->state = AVE_STATE_IDLE;
...
{
	/* IDLE means the queues were never created or are already gone */
	if (session->state == AVE_STATE_IDLE)
		return;

	pr_debug("=== SESSION TEARDOWN START ===\n");

	/*
	 * Skip the goodbye commands for a session that already died: ERROR
	 * means a command or frame timed out, so (a) the T2 is likely not
	 * answering and each command would burn its full 10s budget, and
	 * (b) an unanswered Q1 submission leaves q1 head permanently behind
	 * tail, making every later ave_wait_q1() time out even against a
	 * recovered T2. The queue flushes in ave_queues_destroy() are the
	 * real cleanup; the XPC stop/recover cycle resets the firmware side.
	 */
	if (session->state != AVE_STATE_ERROR) {
		...
		pr_debug("[teardown] sending CompleteFrames (0x03)\n");
		...
		pr_debug("[teardown] sending EndSession (0x06)\n");
...
int ave_session_set_bitrate(struct ave_session *session, u32 bitrate)
{
	...
	if (status == -ETIMEDOUT) {
		/* No response strands a Q1 buffer and desyncs the response
		 * accounting — the session cannot be trusted anymore. */
...
/*
 * Scan length-prefixed encoder output for an IRAP NAL (HEVC types 16-21:
 * BLA/IDR/CRA — the stream sync points). The firmware emits keyframes on
 * its own GOP schedule (MaxKeyFrameInterval), not only when the host
 * requests one, so keyframe-ness must be detected from the bitstream.
 */
static bool ave_output_has_irap(const void *src, size_t size)
{
	const u8 *in = src;
	size_t pos = 0;
	...
	while (pos + 4 <= size) {
		...
		/* The first VCL NAL (types 0-31) decides the whole access
		 * unit; only non-VCL NALs (parameter sets, SEI) precede it */
		if (nal_type < 32)
...
/*
 * Error path helper: a timeout means the T2 never acknowledged the frame,
 * so it may still be DMA-reading the Y/UV plane mappings. Flush the submit
 * queue first — this tells the firmware to drop in-flight submissions —
 * then unmap. Mirrors what ave_queues_destroy() does at teardown.
 */
static void ave_encode_fail_cleanup(struct ave_session *session,
				    struct bce_dma_buffer *y_dma, bool y_mapped,
				    struct bce_dma_buffer *uv_dma, bool uv_mapped)
{
...
/*
 * Shared front half of both frame pipelines: submit the (pre-built)
 * EncodeFrame command, both plane mappings, and the paired Q1 receive
 * buffer. On failure the caller's fail path unmaps whatever the mapped
 * flags say was mapped.
 */
static int ave_submit_encode_frame(struct ave_session *session, const char *tag,
			      ...
			      size_t *encoded_size, bool *keyframe_out)
{
	...
	status = ave_submit_encode_frame(session, "frame1", y_data, y_size,
					 uv_data, uv_size,
	...
	/* Q3 #1: hvcC config (~40ms after submit). This is the first real
	 * work the freshly-started firmware service performs — give it the
	 * full response budget rather than the tight per-frame one. */
	pr_debug("[frame1] waiting for Q3 #1 (hvcC)...\n");
	status = ave_wait_q3(q, 1, AVE_RESPONSE_TIMEOUT_MS);
	if (status) {
	...
	status = ave_wait_q3(q, 2, AVE_FRAME_TIMEOUT_MS);
	if (status) {
	...
	status = ave_wait_q3(q, 3, AVE_FRAME_TIMEOUT_MS);
	if (status) {
	...
	status = ave_wait_q1(q, AVE_FRAME_TIMEOUT_MS);
	if (status) {
	...
	/* Frame 1 opens the stream — always an IDR */
	*keyframe_out = true;
...
resubmit:
	...
	ave_resubmit_q3_bufs(q, ave_q3_event_count(q));
	return 0;

fail:
	ave_encode_fail_cleanup(session, &y_dma, y_mapped, &uv_dma, uv_mapped);
			      ...
			      size_t *encoded_size, bool *keyframe_out)
{
	...
	status = ave_submit_encode_frame(session, "frameN", y_data, y_size,
					 uv_data, uv_size,
					 &y_dma, &y_mapped, &uv_dma, &uv_mapped);
	if (status)
		goto fail;
	...
	status = ave_wait_q3(q, 1, AVE_FRAME_TIMEOUT_MS);
	if (status) {
	...
	/* Detect sync points from the bitstream itself: the firmware also
	 * generates keyframes on its own GOP schedule, not only on request. */
	q3_data = ave_q3_completed_data(q, 1);
	is_irap = ave_output_has_irap(q3_data, raw_size);
	*keyframe_out = is_irap;

	/* Prepend Annex B header (VPS+SPS+PPS) for every sync point so the
	 * stream is joinable at any keyframe */
			     ...
			     bool *keyframe_out)
{
	...
	*keyframe_out = false;
	...
	/* Build EncodeFrame command. The cookie is opaque to the firmware
	 * and merely echoed back — use the frame number, never a kernel
	 * pointer (which would leak kernel addresses to the device). */
